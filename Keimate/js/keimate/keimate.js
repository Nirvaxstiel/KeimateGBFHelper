...
function showBossHP(){
	...
	var enemies = document.querySelectorAll("div.btn-enemy-gauge.prt-enemy-percent.alive");
	enemies.forEach(function(enemy,eIndex){			
		var newStr =  numberWithCommas("		"+a.param[0].hp+"/"+a.param[0].hpmax);
		if(enemy.childNodes.length<3){									
			var newSpan = document.createElement("span");  
			newSpan.innerText = newStr;
			newSpan.style.fontSize = "8px";
			newSpan.paddingLeft = "5px";
			enemy.appendChild(newSpan);
		}else{
			enemy.childNodes[2].innerText = newStr;
		}
	});
}
...
//var target = document.querySelectorAll('DIV.prt-command');
...
var ready = function () {	
	if (document.querySelectorAll('[ability-id]').length >0 &&
		document.querySelectorAll("div.btn-enemy-gauge.prt-enemy-percent.alive").length > 0) {
			
		document.querySelectorAll('[ability-id]').forEach(function(val,index){
			observer.observe(document.querySelectorAll('[ability-id]')[index], config);
		});
